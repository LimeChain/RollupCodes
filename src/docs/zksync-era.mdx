...
<Section title="System Contracts">
    ...
    | :--- | :--- | :--- |
    | <Copy label="0x8002" value="0x8002" /> | [AccountCodeStorage](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/AccountCodeStorage.sol) | Stores a mapping of modified versions of code hashes and account addresses. It is used by the custom compilers in order to simulate extcodehash and extcodesize opcodes. |
    | <Copy label="0x8003" value="0x8003" /> | [NonceHolder](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/NonceHolder.sol) | Along with `bootloader` this contract ensures that the pair (sender, nonce) is always unique, thus ensuring the uniqueness of transaction hashes. Can be used to provide nonce-related information for an account. |
    | <Copy label="0x8004" value="0x8004" /> | [KnownCodesStorage](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/KnownCodesStorage.sol) | Maintains a mapping of known code hashes and additional metadata. It is mostly consumed by other system contracts but can provide bytecode-related information for a given address. |
    | <Copy label="0x8005" value="0x8005" /> | [ImmutableSimulator](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/ImmutableSimulator.sol) | Serves as a utility contract for the custom compiler. It stores immutable variables created during deployment by other contracts. It is needed so that contracts with the same solidity code but different constructors parameters have the same bytecode. |
    | <Copy label="0x8006" value="0x8006" /> | [ContractDeployer](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/ContractDeployer.sol) | Provides functions for deploying smart contracts on the network as well as retrieving account-related information. It is utilised by the custom compilers in order to simulate `create` and `create2` opcodes. |
    | <Copy label="0x8008" value="0x8008" /> | [L1Messenger](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/L1Messenger.sol) | Provides functionality for sending arbitrary-length messages to L1. |
    | <Copy label="0x800a" value="0x800a" /> | [L2EthToken](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/L2EthToken.sol) | Used by the bootloader and `MsgValueSimulator` and `ContractDeployer` system contracts to perform balance changes while simulating the `msg.value` behaviour on Ethereum. |
    | <Copy label="0x800b" value="0x800b" /> | [SystemContext](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/SystemContext.sol) | Provides context variables such as `chainId`, `tx.origin`, `gasPrice`, `blockGasLimit` etc. It is utilised by the custom compiler to provide the implementation of block-related opcodes. |
    | <Copy label="0x800c" value="0x800c" /> | [BootloaderUtilities](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/BootloaderUtilities.sol) | Provides utility functions for computing transaction hashes for a given transaction data depending on its transaction type. It is utilised by the Bootloader contract. |
