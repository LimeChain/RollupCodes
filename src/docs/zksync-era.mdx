...
<Section title="General">
    ...
    <br />
    - ###### L2 → L1
        <Parameter name="Latency" value="TODO minutes" tooltip="The time it takes for a message to be made available on Ethereum after being included in a zkSync Era block and sequenced to L1" />
        <Parameter name="Cost" value="TODO L1 gas cost" tooltip="The gas cost of proving and executing a message on Ethereum after being sent from zkSync Era" />
...
<Section title="OPCODEs">
    ...
    | :--- | :----- | :--- | :-------------------| :--------------- | :-------------------- |
    | ![link icon](../images/link-icon.png) | 3C | EXTCODECOPY | | Not supported. <br /><br />zkSync Era does not provide a mechanism to access the bytecode of a contract.<br /><br />Produces a compile-time error with the custom compiler. | Copies an account’s code to memory. <Unsupported /> |
    ...
    | ![link icon](../images/link-icon.png) | FF | SELFDESTRUCT | | Not supported. <br /><br />Produces a compile-time error with the custom compiler. | Halts execution and registers account for later deletion<br /><br />Deprecated with [EIP-6059](https://eips.ethereum.org/EIPS/eip-6049) <Unsupported /> |
    ...
    | ![link icon](../images/link-icon.png) | 42 | TIMESTAMP | | Returns the latest L1 batch timestamp | Returns the latest block’s timestamp <Modified /> |
    | ![link icon](../images/link-icon.png) | 43 | NUMBER | | Returns the latest L1 batch number | Returns the latest block’s number <Modified /> |
    ...
    | ![link icon](../images/link-icon.png) | 48 | BASEFEE | | Returns `250000000` (wei), however under very high L1 gas prices it may rise. | Returns the base fee <Modified />  |
    | ![link icon](../images/link-icon.png) | 51 | MLOAD | | Loads word from memory.<br /><br />Memory growth is counted in bytes. Payment fees for memory growth are charged linearly at a rate of 1 erg per byte.<br /><br /> [zksolc](https://github.com/matter-labs/zksolc-bin) and [zkvyper](https://github.com/matter-labs/zkvyper-bin) compilers may produce different `msize` compared to Ethereum's `solc` since fewer bytes have been allocated. This may lead to cases where the EVM panics, but ZK Sync EVM does not due to the difference in memory growth. | Load word from memory.<br /><br />Memory growth is in words (chunks). Payment for memory grows quadratically. <Modified /> |
    ...
    | ![link icon](../images/link-icon.png) | F4 | DELEGATECALL | | Message-call into this account with an alternative account’s code, but persisting the current values for sender and value. <br /><br /> The OPCODE does not support sending ether natively <br /><br /> If not compiled with zk-solc or zk-vyper, Ether must be send using a system contract `MsgValueSimulator` prior to executing the `CALL` opcode.<br /><br /> zk-solc and zk-vyper compilers inject the call to the system contract under the hood during compilation. <br /><br /> Memory growth occurs after the call has ended, leading to a difference in the memory size (`msize`). As a result, zkSync Era may not panic in situations where EVM would panic due to differences in memory growth. | Message-call into this account with an alternative account’s code, but persisting the current values for sender and value.<br /><br />Memory growth occurs before the call itself. <Modified /> |
    | ![link icon](../images/link-icon.png) | FA | STATICCALL | | Executes a static message call into an account.<br /><br />Memory growth occurs after the call has ended, leading to a difference in the memory size (`msize`). As a result, zkSync Era may not panic in situations where EVM would panic due to differences in memory growth. | Executes a static message call into an account.<br /><br />Memory growth occurs before the call itself. <Modified /> |
...
<Section title="System Contracts">
    ...
    | :--- | :--- | :--- | :--- |
    | ![link icon](../images/link-icon.png) | 0x8002 <Copy value="0x8002" /> | [AccountCodeStorage](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/AccountCodeStorage.sol) | Stores a mapping of modified versions of code hashes and account addresses. It is used by the custom compilers in order to simulate extcodehash and extcodesize opcodes. |
    | ![link icon](../images/link-icon.png) | 0x8003 <Copy value="0x8003" /> | [NonceHolder](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/NonceHolder.sol) | Along with `bootloader` this contract ensures that the pair (sender, nonce) is always unique, thus ensuring the uniqueness of transaction hashes. Can be used to provide nonce-related information for an account. |
    | ![link icon](../images/link-icon.png) | 0x8004 <Copy value="0x8004" /> | [KnownCodesStorage](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/KnownCodesStorage.sol) | Maintains a mapping of known code hashes and additional metadata. It is mostly consumed by other system contracts but can provide bytecode-related information for a given address. |
    | ![link icon](../images/link-icon.png) | 0x8005 <Copy value="0x8005" /> | [ImmutableSimulator](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/ImmutableSimulator.sol) | Serves as a utility contract for the custom compiler. It stores immutable variables created during deployment by other contracts. It is needed so that contracts with the same solidity code but different constructors parameters have the same bytecode. |
    | ![link icon](../images/link-icon.png) | 0x8006 <Copy value="0x8006" /> | [ContractDeployer](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/ContractDeployer.sol) | Provides functions for deploying smart contracts on the network as well as retrieving account-related information. It is utilised by the custom compilers in order to simulate `create` and `create2` opcodes. |
    | ![link icon](../images/link-icon.png) | 0x8008 <Copy value="0x8008" /> | [L1Messenger](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/L1Messenger.sol) | Provides functionality for sending arbitrary-length messages to L1. |
    | ![link icon](../images/link-icon.png) | 0x800a <Copy value="0x800a" /> | [L2EthToken](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/L2EthToken.sol) | Used by the bootloader and `MsgValueSimulator` and `ContractDeployer` system contracts to perform balance changes while simulating the `msg.value` behaviour on Ethereum. |
    | ![link icon](../images/link-icon.png) | 0x800b <Copy value="0x800b" /> | [SystemContext](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/SystemContext.sol) | Provides context variables such as `chainId`, `tx.origin`, `gasPrice`, `blockGasLimit` etc. It is utilised by the custom compiler to provide the implementation of block-related opcodes. |
    | ![link icon](../images/link-icon.png) | 0x800c <Copy value="0x800c" /> | [BootloaderUtilities](https://github.com/matter-labs/era-system-contracts/blob/main/contracts/BootloaderUtilities.sol) | Provides utility functions for computing transaction hashes for a given transaction data depending on its transaction type. It is utilised by the Bootloader contract. |
...
<Section title="RPC-API">
    ...
    | ![link icon](../images/link-icon.png) | `eth_getBlockByNumber` | Hex-encoded integer | Returns information for a given block. <br /><br />Adds additional information such as l1 batch number and timestamp. | Returns information for a given block. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getBlockByHash` | Hash | Returns information for a given block. <br /><br />Adds additional information such as l1 batch number and timestamp. | Returns information for a given block. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getTransactionByHash` | Hash | Returns information for a given transaction. <br /><br />Adds additional information such as l1 batch number and timestamp. | Returns information for a given transaction. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getTransactionByBlockHashAndIndex` | Hash and hex-encoded integer | Returns information for a given transaction. <br /><br />Adds additional information such as l1 batch number and timestamp. | Returns information for a given transaction. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getTransactionByBlockNumberAndIndex` | Two hex-encoded integers | Returns information for a given transaction. <br /><br />Adds additional information such as l1 batch number and timestamp. | Returns information for a given transaction. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getTransactionReceipt` | Hash | Returns information for a given transaction. <br /><br />Adds additional information such as l1 batch number, l1 timestamp, l1 batch TX index and L2toL1 logs. | Returns information for a given transaction. <Modified /> |
