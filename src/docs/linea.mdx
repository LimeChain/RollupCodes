...
<Section title="RPC-API">
    ...
    | :----- | :----- | :--------------- | :-------------------- |
    | `eth_newBlockFilter` | None | Implemented in Node, not available in providers yet  | Creates a filter in the node, to notify when a new block arrives. <Unsupported /> |
    | `eth_newFilter` | None | Implemented in Node, not available in providers yet  | Creates a filter object, based on filter options, to notify when the state changes (logs). <Unsupported /> |
    
</Section>

<Section title="Transaction Fees">

    Transaction fees are paid in Ether. 
    
    There is no concept of an L1 calldata fee commonly used in other rollups.
    
    Developers must set gasPrice high enough so that the sequencer would include the transaction in a batch.
    
    Information on the gas price can be retrieved from the JSON RPC endpoint.
