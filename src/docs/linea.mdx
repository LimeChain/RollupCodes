...
<Section title="RPC-API">
    ...
    | `eth_newFilter` | Filter object | Not supported by public nodes. Supported when running your own follower node  | Creates a filter object, based on filter options, to notify when the state changes (logs). <Unsupported /> |
