---
title: "Linea"
subtitle: "Linea is a ZK-Rollup powered by Consensys zkEVM. The protocol team centrally operates the coordinator (sequencer + prover) which is closed source. They are utilizing quantum-resistant cryptography for their zk-proofs. The rollup has rich tooling and infrastructure support."
labels:
  - ZK Rollup
  - EVM
  - Mainnet
links:
  website:
    url: "https://linea.build/"
    name: "linea.build"
  docs:
    url: "https://docs.linea.build/"
    name: "Docs"
  l2beat:
    url: "https://l2beat.com/scaling/projects/linea"
    name: "L2Beat"
  github:
    url: "https://github.com/Consensys/linea-contracts"
    name: "Github"
  twitter:
    url: "https://twitter.com/LineaBuild"
    name: "LineaBuild"
  discord:
    url: "https://discord.com/invite/linea"
    name: "Discord"
---

<Section title="Overview">
Linea is a ZK-Rollup powered by Consensys zkEVM. The protocol team centrally operates the coordinator (sequencer + prover) which is closed source. They are utilizing quantum-resistant cryptography for their zk-proofs. The rollup has rich tooling and infrastructure support.

###### Focus
- Open sourcing rollup operators
- Support for multiple zk-proof algorithms

<Labels labels={['ZK Rollup', 'EVM', 'Mainnet']} title="Linea" />
</Section>

<Section title="General">

    <Parameter name="Block Time" value="12 seconds" tooltip="The rate at which the rollup produces blocks. Keep in mind that the value is subject to change in the future"/>

    <Parameter name="Gas Limit" value="61 million" tooltip="The gas limit that can be consumed by an L2 block"/>

    <Parameter name="Sequencing Frequency" value="30 - 120 seconds" tooltip="The frequency at which the rollup posts L2 transactions on Ethereum L1." />

    <Parameter name="Finality" value="1 - 32 hours" tooltip="The time it takes for an L2 transaction to be considered final after it is sequenced on L1" />

    <Parameter name="Rollup’s State Contract on L1" value={<Reference label="0xd19d4B5d358258f05D7B411E21A1460D11B0876F" url="https://etherscan.io/address/0xd19d4B5d358258f05D7B411E21A1460D11B0876F" />} tooltip="The contract used for sequencing, proving and storing the state of the L2 network" />

    <Parameter name="State Trie" value="Sparse Merkle Trie + MiMC Hashing" tooltip="The data structure used to represent the state of the Rollup along with the hashing algorithm used to compute the root of the trie" />

    <Parameter name="Node Implementations" value={"Geth is the primary node implementation. The production Sequencer and Prover are closed-source, linea-besu is an open-sourced node under development."} tooltip="The different client implementations of the Rollup" />

    <Parameter name="Transaction Types" tooltip="The types of transactions supported on the Rollup" />

        - **Type 0** - User Transaction. Represent [pre-EIP-2718](https://eips.ethereum.org/EIPS/eip-2718) transactions
        - **Type 1** - User Transaction. Represent legacy transactions post [EIP-2718](https://eips.ethereum.org/EIPS/eip-2718) transactions
        - **Type 2** - User Transaction. [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559) transactions
    ___

    <MultiRowParameters title="Messaging" tooltip="The Rollup provides native communication with L1. Anyone is able to transmit arbitrary messages and the Arbitrum protocol will provide proof of the data transfer on the destination chain" data={[
        {
            title: 'L1 → L2 (automatic)',
            rows: [
                {
                    label: 'Latency',
                    tooltip: 'The time it takes for a message to be made available on the Rollup after it is included in the Ethereum block.',
                    value: '17min'
                },
                {
                    label: 'Cost',
                    tooltip: 'The gas cost of purely sending a message from Ethereum to Linea. Senders on L1 pay an additional fee for the execution of the message on L2.',
                    value: '66 000 L1 gas'
                },
                {
                    label: 'Delivery method',
                    tooltip: 'How the smart contract on the target chain is called. In both automatic and manual claiming we are waiting for a central unit to add the message hashes on the target chain (~17 minutes)',
                    value: 'Automatic'
                },
            ]
        },
        {
            title: 'L1 → L2 (manual)',
            rows: [
                {
                    label: 'Latency',
                    tooltip: 'The time it takes for a message to be made available on the Rollup after it is included in the Ethereum block.',
                    value: '> 17 min'
                },
                {
                    label: 'Cost',
                    tooltip: 'The gas cost of purely sending a message from Ethereum to Linea plus the actual execution that is scheduled to take place on the L2. Senders on L1 pay for the execution of the message on L2.',
                    value: '66 000 L1 gas + 71 000 L2 gas'
                },
                {
                    label: 'Delivery method',
                    tooltip: 'How the smart contract on the target chain is called. In both automatic and manual claiming we are waiting for a central unit to add the message hashes on the target chain (~17 minutes)',
                    value: 'Manual'
                }
            ]
        },
        {
            title: 'L2 → L1',
            rows: [
                {
                    label: 'Latency',
                    tooltip: "The time it takes for a message to be made available on Ethereum after being included in an Linea block and sequenced to L1. Messages can only be claimed after ZK proof has been submitted on L1 that verifies it's been submitted on L2.",
                    value: '1 to 32 hours'
                },
                {
                    label: 'Cost',
                    tooltip: 'The gas cost of executing a message on Ethereum after being sent from Linea. The fee for the prover is paid by the sender on L2.',
                    value: '73 000 L2 gas'
                },
                {
                    label: 'Delivery method',
                    tooltip: 'How the smart contract on the target chain is called.',
                    value: 'Manual'
                },
            ],
        },
    ]} />

</Section>

<Section title="OPCODEs">

    <Legend />

    Linea uses the London version of the EVM.

    The following table encapsulates EVM OPCODE differences of the Rollup compared to the canonical Ethereum L1 implementation.

    | Opcode | Name | Solidity Equivalent | Rollup Behaviour | Ethereum L1 Behaviour |
    | :----- | :--- | :-------------------| :--------------- | :-------------------- |
    | 44 | PREVRANDAO (legacy DIFFICULTY) | `block.difficulty` | Returns a fixed number "2" | Get the output of the randomness beacon provided by the beacon chain <Modified /> |
    | 5F | PUSH0 |  | Not supported yet! <br /><br /> Solidity version `0.8.20` or higher can only be used with an evm version lower than the default `shanghai`. <br /><br /> Versions up to `0.8.19` (included) are fully compatible | Places value 0 on the stack <Unsupported /> |

</Section>

<Section title="Precompiled Contracts">

    > ⚠️ All Precompiled Contacts defined in the canonical Ethereum L1 implementation (until London hard fork) have the same behaviour on the Rollup, however, the execution of those Precompiles is not covered by the circuits and therefore not present in the Validity proof

</Section>

<Section title="System Contracts">

    The Rollup has introduced the following system contracts.

    | Address | Name | Description |
    | :--- | :--- | :--- |
    | <Copy value="0xC84832f69bFFbC1A94E44a157A342766E959ED27" label="0xC84...27" /> | <Reference label="PlonkVerifierFull" url="https://github.com/Consensys/linea-contracts/blob/main/contracts/verifiers/PlonkVerifierFull.sol" /> | Plonk verifier contract used by the Linea zkEVM rollup. |
    | <Copy value="0xb32c3D0dDb0063FfB15E8a50b40cC62230D820B3" label="0xb32...B3" /> | <Reference label="ZkEvmV2" url="https://github.com/Consensys/linea-contracts/blob/main/contracts/ZkEvmV2.sol" /> |The main contract of the Linea zkEVM rollup. Contains state roots, the verifier addresses and manages messages between L1 and the L2.|
    | <Copy value="0xd19d4B5d358258f05D7B411E21A1460D11B0876F" label="0xd19...6F" /> | <Reference label="L1 Message Service" url="https://github.com/Consensys/linea-contracts/blob/main/contracts/messageService/l1/L1MessageService.sol" /> | L1 Message Service.|
    | <Copy value="0x508Ca82Df566dCD1B0DE8296e70a96332cD644ec" label="0x508...ec" /> | <Reference label="L2 Message Service" url="https://github.com/Consensys/linea-contracts/blob/main/contracts/messageService/l2/L2MessageService.sol" /> | L2 Message Service.|

</Section>

<Section title="RPC-API">

    <Legend />

    The following endpoints behave differently compared to the canonical Ethereum L1 implementation of the JSON RPC API.

    | Method | Params | Rollup Behaviour | Ethereum L1 Behaviour |
    | :----- | :----- | :--------------- | :-------------------- |
    | `eth_newBlockFilter` | None | Not supported  | Creates a filter in the node, to notify when a new block arrives. <Unsupported /> |
    | `eth_newFilter` | None | Not supported  | Creates a filter object, based on filter options, to notify when the state changes (logs). <Unsupported /> |
    
</Section>

<Section title="Transaction Fees">

    Other than the L1 gas fee component, Linea charges L2 gas fees which should be around 1/15th of Ethereum's.

    [L2 Gas fees](https://docs.linea.build/build-on-linea/gas-fees).

</Section>

<Section title="Tools">

    Existing EVM-based tooling is supported such as ethers, web3.js, ethers-rs, hardhat, foundry and truffle.

    [Linea SDK](https://docs.linea.build/build-on-linea/linea-sdk) focuses on interacting with smart contracts on both Ethereum and Linea networks and provides custom functions to obtain message information.

    [Linea Safe](https://safe.linea.build/welcome) is a white-label fork of Gnosis Safe deployed by the Linea team.

    [Ape framework](https://github.com/Consensys/ape-linea) is a plugin to add support to the Ape cli web3 development tool for Linea developed by the Linea team.

</Section>
