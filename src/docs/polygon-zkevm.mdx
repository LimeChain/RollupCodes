...
<Section title="OPCODEs">
    ...
    | ![link icon](../images/link-icon.png) | F3 | EXTCODEHASH | | Returns the Poseidon hash of the contract bytecode from the state tree. <br /><br /> Returns empty hash even if the account does not exist. | Returns the Poseidon hash of the contract bytecode from the state tree.<br /><br />Returns empty hash even if the account does not exist. <Modified /> |
...
<Section title="Precompiled Contracts">
    ...
    | :--- | :----- | :----- | :--------------- | :-------------------- |
    | ![link icon](../images/link-icon.png) | 0x02 <Copy value="0x02" /> | `SHA2-256` | **Not supported**<br /><br />The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Elliptic curve digital signature algorithm (ECDSA) public key recovery function. <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x03 <Copy value="0x03" /> | `RIPEMD-160` | **Not supported**<br /><br />The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Hash function <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x05 <Copy value="0x05" /> | `modexp` | **Not supported**<br /><br />The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Arbitrary-precision exponentiation under modulo <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x06 <Copy value="0x06" /> | `ecAdd` | **Not supported**<br /><br />The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Point addition (ADD) on the elliptic curve `alt_bn128` <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x07 <Copy value="0x07" /> | `ecMul` | **Not supported**<br /><br />The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Scalar multiplication (MUL) on the elliptic curve `alt_bn128` <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x08 <Copy value="0x08" /> | `ecPairing` | **Not supported**<br /><br />The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Bilinear function on groups on the elliptic curve `alt_bn128` <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x09 <Copy value="0x09" /> | `blake2f` | **Not supported**<br /><br />The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Compression function F used in the BLAKE2 cryptographic hashing algorithm <Unsupported /> |
