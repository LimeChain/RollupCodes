...
<Section title="OPCODEs">
    ...
    | :--- | :----- | :--- | :-------------------| :--------------- | :-------------------- |
    | ![link icon](../images/link-icon.png) | FF | SELFDESTRUCT | `selfdestruct(address)` | Removed and replaced by `SENDALL` as per [EIP-4758](https://eips.ethereum.org/EIPS/eip-4758) | Halts execution and registers an account for later deletion. Sends all Ether balance to the specified address. <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 3F | EXTCODEHASH | | Returns the Poseidon hash of the contract bytecode from the state tree. <br /><br /> Returns empty hash even if the account does not exist. | Returns the Keccak hash of the contract bytecode from the state tree.<br /><br />Returns empty hash if the account has no code and 0 if the account does not exist or has been destroyed.<Modified /> |
    | ![link icon](../images/link-icon.png) | 40 | BLOCKHASH | `blockhash(x)` | Returns the hash of all previous L2 block hashes instead of just the last 256 blocks.<br /><br />It is the state root at the end of the processable transaction and is stored on the system smart contract. | Returns the hash of one of the 256 most recent complete blocks. <Modified /> |
    | ![link icon](../images/link-icon.png) | 43 | NUMBER | `block.number` | Blocks on the rollup are created per transaction.<br /><br />The OPCODE returns the number of processed transactions. | Returns the L1 block number. <Modified /> |
    | ![link icon](../images/link-icon.png) | 44 | DIFFICULTY | `block.difficulty` | Always returns `0` | Returns the output of the randomness beacon provided by the beacon chain. <Modified /> |
...
<Section title="Precompiled Contracts">
    ...
    | :--- | :----- | :----- | :--------------- | :-------------------- |
    | ![link icon](../images/link-icon.png) | 0x02 <Copy value="0x02" /> | `SHA2-256` | The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Elliptic curve digital signature algorithm (ECDSA) public key recovery function. <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x03 <Copy value="0x03" /> | `RIPEMD-160` | The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Hash function <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x05 <Copy value="0x05" /> | `modexp` | The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Arbitrary-precision exponentiation under modulo <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x06 <Copy value="0x06" /> | `ecAdd` | The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Point addition (ADD) on the elliptic curve `alt_bn128` <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x07 <Copy value="0x07" /> | `ecMul` | The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Scalar multiplication (MUL) on the elliptic curve `alt_bn128` <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x08 <Copy value="0x08" /> | `ecPairing` | The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Bilinear function on groups on the elliptic curve `alt_bn128` <Unsupported /> |
    | ![link icon](../images/link-icon.png) | 0x09 <Copy value="0x09" /> | `blake2f` | The opcode reverts the execution, returns all gas to the previous context and sets the `success` flag to `0` | Compression function F used in the BLAKE2 cryptographic hashing algorithm <Unsupported /> |
...
<Section title="RPC-API">
    ...
    | ![link icon](../images/link-icon.png) | `eth_getBalance` | Address and integer block number (or the string `latest`, `earliest` or `pending`) | Returns the balance of the account of the given address.<br /><br />If the block number is set to `pending` it is replaced and computed with `latest` | Returns the balance of the account of the given address. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getCode` | Address and integer block number (or the string `latest`, `earliest` or `pending`) | Returns code at a given address.<br /><br />If the block number is set to `pending` it is replaced and computed with `latest` | Returns code at a given address. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getUncleByBlockHashAndIndex` | Block hash and integer index | Response is always empty | Returns information about a uncle of a block by hash and uncle index position. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getUncleByBlockNumberAndIndex` | Block number (integer) and integer index | Response is always empty | Returns information about a uncle of a block by number and uncle index position. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getUncleCountByBlockHash` | Block hash | Response is always zero. | Returns the number of uncles in a block from a block matching the given block hash. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_getUncleCountByBlockNumber` | Block number | Response is always zero. | Returns the number of uncles in a block from a block matching the given block number. <Modified /> |
    | ![link icon](../images/link-icon.png) | `eth_protocolVersion` | None | Response is always zero. | Returns the current Ethereum protocol version. <Modified /> |
    | ![link icon](../images/link-icon.png) | `txpool_content` | None | Response is always empty | N/A <Added /> |
    | ![link icon](../images/link-icon.png) | `zkevm_batchNumber` | None | Returns the latest batch number | N/A <Added /> |
    | ![link icon](../images/link-icon.png) | `zkevm_batchNumberByBlockNumber` | Block number | Returns the batch number of the batch connected to the block. | N/A <Added /> |
