---
title: Soneium
subtitle: Soneium is an Optimistic Rollup utilizing OP Stack technology. The protocol features a centralised sequencer operated by the Soneium team.
labels:
    - Optimistic Rollup
    - EVM
    - Mainnet
links:
    ...
    docs:
        ...
        name: Docs
    l2beat:
        url: https://l2beat.com/scaling/projects/soneium
        name: L2BEAT
    github:
...
<Section title="Overview">
   Soneium is an Optimistic Rollup utilizing OP Stack technology. The protocol features a centralised sequencer operated by the Soneium team.

    ###### Focus

    - Mainstream Adoption of Web3 by leveraging partnerships with Sony and Startale to lower barriers to entry and promote widespread use of Web3 solutions
    - Interoperability with the OP Superchain

    <Labels labels={['Optimistic Rollup', 'EVM', 'Mainnet']} title="Soneium" />

</Section>

<Section title="General">

    <Parameter name="Block Time" value="2 second" tooltip="The rate at which the rollup produces blocks. Keep in mind that the value is subject to change in the future" />

    <Parameter name="Gas Limit" value="40 million" tooltip="The gas limit that can be consumed by an L2 block" />

    <Parameter name="Gas Target" value="5.7 million" tooltip="The EIP-1559 gas target for an L2 block" />

    <Parameter name="Sequencing Frequency" value="15-20 minutes" tooltip="The frequency at which the rollup posts L2 transactions on Ethereum L1. The time varies based on the amount of calldata that must be posted on L1" />
    ...
    <Parameter name="Node Implementations" value={[<Reference key="0" label="op-geth (Go)" url="https://github.com/ethereum-optimism/op-geth" />]} tooltip="The different client implementations of the Rollup" />
    ...
    <MultiRowParameters title="Messaging" tooltip="The Rollup provides native communication with L1. Anyone is able to transmit arbitrary messages and the Optimism protocol will provide proof of the data transfer on the destination chain" data={[
        {
            ...
            rows: [
                ...
                {
                    label: 'Cost',
                    tooltip: 'The gas cost of sending a message from Ethereum to the L2. Soneium executes the message with a system address on behalf of the L1 sender. L2 gas is paid by the sender of the message on L1.',
        ...
        },
        {
            ...
            rows: [
                {
                    label: 'Latency',
                    tooltip: 'The time it takes for a message to be made available on Ethereum after being included in a Soneium block and sequenced to L1.',
                    value: '7 days',
                },
                {
                    label: 'Cost',
                    tooltip: 'The gas cost of purely proving and triggering the execution of the message on Ethereum after being sent from Soneium. It is paid by the EOA that initiates the 2 transaction on L1: prove and finalize. The gas cost does not include the cost from the logic triggered by the message.',
...
<Section title="Transaction Fees">
    ...
    Soneium charges an additional L1 data fee. You can use [@eth-optimism/sdk](https://sdk.optimism.io/modules.html#asL2Provider)
