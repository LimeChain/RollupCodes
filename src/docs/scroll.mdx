...
<Section title="Transaction Fees">
    ...
    The rollup fee (previously L1 data fee) is deducted directly from the sender’s balance before a transaction’s execution, not paid for using gas. Galileo replaces the previous step-function penalty model with pricing based on the transaction's compressed size and applies a quadratic penalty for large, poorly compressible transactions. Developers can use the [L1GasOracle](https://scrollscan.com/address/0x5300000000000000000000000000000000000002) predeployed contract to estimate the rollup fee.

    Galileo rollup fee formula is `galileo_l1_fee(tx) = fee_per_byte * compressed_size(tx) * (1 + blob_utilization_penalty(tx))`
</Section>
